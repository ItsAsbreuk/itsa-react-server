...
const fs = require('fs-extra'),
    ...
    nonCssVersionedExternals = reload('../find-noncss-versioned-externals')(),
...
const copyExternalNonCssPackages = () => {
    let list = [];
    nonCssVersionedExternals.forEach(item => list.push(fs.copy(path.resolve(cwd, 'node_modules', item.module), path.resolve(cwd, 'build.tmp/public/assets/_itsa_server_external_modules/'+item.version+'/', item.module), {overwrite: true})));
    return Promise.resolve(list);
};
...
const buildWebpack = (environment, processtime, sourceMaps) => {
    ...
    promise = new Promise(async (resolve, reject) => {
        try {
            ...
            cancelled || (await copyExternalNonCssPackages());
